...
// username = await getUserInput("What is your name?"); // COMENTADO para empezar el juego directo.
...
// 1. ARRAY DE PALABRAS (> 6 caracteres)
const palabras = [
...
async function startGame(){
    ...
    console.log(`\nHola ${username}, ¡bienvenido al Ahorcado!`);
...
async function bucleJuego() {
    while (intentos > 0 && palabraMostrada.includes("_")) {
        ...
        const letra = (await getUserInput("Introduce una letra:")).toLowerCase();
    ...
    if (!palabraMostrada.includes("_")) {
        victorias++;
        console.log("\n🏆 ¡GANASTE! La palabra era: " + palabraSecreta.toUpperCase());
    } else {
        derrotas++;
        console.log("\n💀 GAME OVER. La palabra era: " + palabraSecreta.toUpperCase());
    }
